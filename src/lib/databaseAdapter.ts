...
export default function RedisAdapter(client: Redis, options = {}): Adapter {
  const mergedOptions = {
    ...defaultOptions,
    ...options,
  };
  ...
  const getStringAsObject = (key: string) => JSON.parse(key);
  ...
  const getAccount = async (id: string) => {
    ...
    return hydrateDates(getStringAsObject(account));
  };
  ...
  const getSession = async (id: string) => {
    ...
    return hydrateDates(getStringAsObject(session));
  };
  ...
  const getUser = async (id: string) => {
    ...
    if (!user) return null;
    return hydrateDates(getStringAsObject(user));
  };
  ...
  return {
    ...
    async useVerificationToken(verificationToken) {
      ...
      return hydrateDates(getStringAsObject(token));
    },
